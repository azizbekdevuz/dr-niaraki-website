...
class Node {
  ...
  draw(context: CanvasRenderingContext2D, hueShift: number, nodeColor: (hue: number) => string) {
    if (!this.active) return;
    
    const gradient = context.createRadialGradient(this.x, this.y, 0, this.x, this.y, 6);
  ...
  update(canvas: HTMLCanvasElement, restrictedArea: {left: number, right: number, top: number, bottom: number}) {
    if (!this.active) return;
...
export default function AdvancedBackground({ theme = 'dark' }: { theme?: Theme }) {
  ...
  const setupCanvas = useCallback((canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) => {
  ...
  const adjustNodeCount = useCallback((canvas: HTMLCanvasElement) => {
  ...
  const drawShape = useCallback((shapeIndex: number, canvas: HTMLCanvasElement) => {
    ...
    shapeNodes.forEach((node, index) => {
      const angle = (Math.PI * 2 * index) / shapeNodeCount;
  
      switch (shapeIndex) {
        case 0: // Circuit Shape
          node.targetX = canvas.width / 2 + (index % 2 === 0 ? radius : -radius);
          node.targetY = canvas.height / 2 + (index % 2 === 0 ? -radius : radius);
          break;
        case 1: // Holographic Display (Rectangular Grid)
          const gridSize = 5;
          node.targetX = canvas.width / 2 + (index % gridSize) * 40 - (gridSize - 1) * 20;
          node.targetY = canvas.height / 2 + Math.floor(index / gridSize) * 40 - (gridSize - 1) * 20;
          break;
        case 2: // Hexagonal Grid
          if (index === 0) {
            ...
            node.targetX = canvas.width / 2 + Math.cos(ringAngle) * (radius * ringIndex / 2);
            node.targetY = canvas.height / 2 + Math.sin(ringAngle) * (radius * ringIndex / 2);
          }
          break;
        case 3: // Concentric Circles
          const ringsCount = 3;
          ...
          node.targetX = canvas.width / 2 + Math.cos(circleAngle) * (50 + 40 * ringIndex);
          node.targetY = canvas.height / 2 + Math.sin(circleAngle) * (50 + 40 * ringIndex);
          break;
        case 4: // Pentagon/Star
        ...
        case 5: // Artificial Neural Network
          ...
          node.targetX = canvas.width / 2 + (layerIndex - (layersCount - 1) / 2) * 100;
          node.targetY = canvas.height / 2 + (nodeInLayerIndex - (nodesPerLayer - 1) / 2) * 60;
          break;
        case 6: // Research Data Network
          node.targetX = canvas.width / 2 + Math.random() * radius * 2 - radius;
          node.targetY = canvas.height / 2 + Math.random() * radius * 2 - radius;
          break;
      }
  ...
  const animate = useCallback((currentTime: number) => {
    ...
    activeNodes.forEach(node => node.update(canvas, restrictedAreaRef.current));
  
    for (let i = 0; i < activeNodes.length; i++) {
      const node = activeNodes[i];
      node.draw(context, hueShiftRef.current, themeColors[theme as Theme].nodeColor);
      ...
      for (let j = i + 1; j < activeNodes.length; j++) {
        ...
        if (distance < maxDistance) {
          ...
          context.strokeStyle = themeColors[theme].lineColor(1 - distance / maxDistance);
          context.stroke();

          const midX = (node.x + otherNode.x) / 2;
          const midY = (node.y + otherNode.y) / 2;
          const gradient = context.createRadialGradient(midX, midY, 0, midX, midY, 20);
          gradient.addColorStop(0, `hsla(${hueShiftRef.current}, 100%, 50%, 0.5)`);
          gradient.addColorStop(1, 'transparent');
          context.fillStyle = gradient;
  ...
  }, [setupCanvas, adjustNodeCount, animate, handleMouseMove, handleResize, resetShapeNodes, drawShape]);

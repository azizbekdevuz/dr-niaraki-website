...
const useScrollHandler = (
  ...
  return useCallback((e: WheelEvent | TouchEvent) => {
    ...
    if ((isAtTop && delta < 0 && currentSection > 0) ||
  ...
  }, [currentSection, isTransitioning, setCurrentSection, setDirection, setIsTransitioning, sectionRefs, transitionDuration, totalSections]);
};
...
const SmoothScrollWrapper: React.FC<SmoothScrollWrapperProps> = ({ 
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      } else if (e.key === 'ArrowDown' && currentSection < children.length - 1) {
        setIsTransitioning(true);
        setDirection(1);
        setCurrentSection(prev => prev + 1);
      }
      ...
      <div className="fixed right-8 top-1/2 transform -translate-y-1/2 z-50 flex flex-col items-center gap-4">
        <button
          ...
          className={`p-2 rounded-full transition-all duration-300 ${
            currentSection === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-200/20'
          }`}
        ...
        {/* Navigation Dots */}
        <div className="flex flex-col gap-4" role="navigation" aria-label="Section navigation">
        ...
        <button
          onClick={() => currentSection < children.length - 1 && setCurrentSection(currentSection + 1)}
          className={`p-2 rounded-full transition-all duration-300 ${
            currentSection === children.length - 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-200/20'
          }`}
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          <div 
            ...
            role="region"
            aria-label={sectionTitles[currentSection] || `Section ${currentSection + 1}`}
          >

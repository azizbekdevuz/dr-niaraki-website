...
import publicationsData, { Publication, PublicationType } from '../components/datasets/pblcDataset';
...
const ResearchDomainEcosystem: React.FC<{ darkMode: boolean }> = ({ darkMode }) => {
        ...
        {domains.map((domain) => {
          ...
          return (
            ...
            >
              ...
              {isSelected && (
                <div className="mt-4">
                  <h4 className="font-medium text-sm mb-2">Key Research Areas:</h4>
                  <ul className={`
...
const ResearchJourneyMilestones: React.FC<{ darkMode: boolean }> = ({ darkMode }) => {
  const milestones = [
    {
      year: 2024,
...
const ComprehensiveResearchExplorer: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'publications' | 'domains' | 'milestones'>('publications');
  ...
  const publicationTags = [
    'Geo-AI', 'Machine Learning', 'XR Technologies', 
    'IoT', 'Spatial Computing', 'Urban Analytics'
  ];
    ...
    return publicationsData.filter(pub => 
      ...
      (selectedTags.length === 0 || 
        selectedTags.some(tag => 
          ...
          pub.authors.some(author => author.toLowerCase().includes(tag.toLowerCase()))
        )
      )
    );
  ...
  const publicationStats = useMemo(() => {
    return {
      ...
      byType: {
        ...
        'book_chapter': publicationsData.filter(p => p.type === 'book_chapter').length,
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8 py-16">
        ...
        <div className="grid grid-cols-3 gap-6 mb-12">
          ...
          `}>
            ...
            <div>
              <h3 className="text-2xl font-bold">{publicationStats.byType.journal}</h3>
        ...
        <div>
            ...
            <>
              ...
              <div className="max-w-7xl mx-auto px-8 mb-6">
                <div className="flex items-center space-x-4">
                  <TagIcon className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`} />
                  <TagFilter 
          ...
          {activeView === 'domains' && <ResearchDomainEcosystem darkMode={darkMode} />}
          
          {activeView === 'milestones' && <ResearchJourneyMilestones darkMode={darkMode} />}
        </div>

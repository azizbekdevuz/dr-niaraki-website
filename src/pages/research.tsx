...
import { ChevronRight, Award, Book, Brain, Microscope, Globe, Sparkles } from "lucide-react";
...
const ResearchCard = React.memo<ResearchCardProps>(({ data, index, type, isActive, darkMode }) => {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        ...
        >
          <Sparkles className={`w-6 h-6 ${isHovered ? "text-blue-400" : ""}`} />
        </motion.div>
      </div>
...
const Research = ({ darkMode = true }: { darkMode?: boolean }) => {
  ...
  const tabs: Tab[] = useMemo(
    () => [
      { id: "projects", label: "Research Projects", icon: Microscope, data: researchProjectsData },
      { id: "leadership", label: "Research Leadership", icon: Brain, data: researchLeadershipData },
      { id: "initiatives", label: "Academic Initiatives", icon: Book, data: academicInitiativesData },
    ],
    []
  );
